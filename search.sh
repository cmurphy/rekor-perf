...
hyperfine --style basic --warmup 10 --runs $RUNS --ignore-failure --parameter-list public_key ${DIR}/test1@example.com.key,${DIR}/test2@example.com.key,${DIR}/notreal@example.com.key "rekor-cli search --rekor_server http://localhost:3000 --public-key {public_key}"
...
hyperfine --style basic --warmup 10 --runs $RUNS --ignore-failure --parameter-list public_key ${DIR}/test1@example.com.key,${DIR}/test2@example.com.key,${DIR}/notreal@example.com.key \
