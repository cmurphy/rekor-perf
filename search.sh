...
hyperfine --warmup 10 --ignore-failure --parameter-list public_key ${DIR}/test1@example.com.key,${DIR}/test2@example.com.key,${DIR}/notreal@example.com.key "rekor-cli search --rekor_server http://localhost:3000 --public-key {public_key}"
# Search for entries using the sha256 sum of blob1 (should be many), blob2 (should be few), blobnone (should be none)
hyperfine --warmup 10 --ignore-failure --parameter-list sha ${sumblob1},${sumblob2},${sumblobnone} "rekor-cli search --rekor_server http://localhost:3000 --sha sha256:{sha}"
...
hyperfine --warmup 10 --ignore-failure --parameter-list public_key ${DIR}/test1@example.com.key,${DIR}/test2@example.com.key,${DIR}/notreal@example.com.key \
