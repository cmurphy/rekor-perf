...
hyperfine --style basic --warmup 10 --ignore-failure --parameter-list public_key ${DIR}/user1@example.com.key,${DIR}/user2@example.com.key,${DIR}/notreal@example.com.key "rekor-cli search --rekor_server $REKOR_URL --public-key {public_key}"
...
hyperfine --style basic --warmup 10 --ignore-failure --parameter-list public_key ${DIR}/user1@example.com.key,${DIR}/user2@example.com.key,${DIR}/notreal@example.com.key \
    ...
    "rekor-cli search --rekor_server $REKOR_URL --public-key {public_key} --sha sha256:{sha} --operator {operator}"
