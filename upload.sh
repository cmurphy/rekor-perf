...
echo "Signing 10 artifacts with 1 key"
user=test1@example.com
for i in $(seq 1 10) ; do
    sig=${dir}/$(uuidgen).asc
    gpg --armor -u $user --output $sig --detach-sig ${dir}/blob${i}
    rekor-cli upload --rekor_server http://localhost:3000 --signature $sig --public-key ${dir}/${user}.key --artifact ${dir}/blob${i}
done

echo "Signing 1 artifact with 10 keys"
for i in $(seq 1 10) ; do
    sig=${dir}/$(uuidgen).asc
    user=test${i}@example.com
...
echo "Artifacts and signatures stored in ${dir}."
